...
class ODBCHandler:
    ...
    def list_locks(self, connection_name: Optional[str] = None) -> Tuple[List[str], List[List[Any]]]:
    
        """
        List current DBMaker locks.
        """
        ...
        return self.execute_query(sql, connection_name, None)
    
    def list_active_users(self, connection_name: Optional[str] = None) -> Tuple[List[str], List[List[Any]]]:
        """
        List current active database users/connections.
        """
    ...
    def health_check(self, connection_name: Optional[str] = None) -> Tuple[List[str], List[List[Any]]]:
        sql = """
            SELECT ID, INFO, VALUE
            FROM SYSINFO
            WHERE ID IN ('0503','0504','0801','0802','0803','0709','0710')
        """

        return self.execute_query(sql, connection_name, None)


    def list_user_stored_procedures(self, connection_name: Optional[str] = None) -> Tuple[List[str], List[List[Any]]]:
        sql = """
            SELECT PROC_OWNER, NAME
            FROM SYSPROCINFO
            WHERE PROC_OWNER <> 'SYSTEM'
        """

        return self.execute_query(sql, connection_name, None)


    def list_system_stored_procedures(self, connection_name: Optional[str] = None) -> Tuple[List[str], List[List[Any]]]:
        sql = """
            SELECT PROC_OWNER, NAME
            FROM SYSPROCINFO
            WHERE PROC_OWNER = 'SYSTEM'
    ...
    def list_triggers(self, connection_name: Optional[str] = None) -> Tuple[List[str], List[List[Any]]]:
        sql = """
            SELECT 
                TBNAME,
                TRIGNAME,
    ...
    def list_foreign_keys(self, connection_name: Optional[str] = None) -> Tuple[List[str], List[List[Any]]]:
        sql = """
            SELECT
                FK_TBL_NAME,
                PK_TBL_NAME,
