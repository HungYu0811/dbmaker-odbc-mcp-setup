...
class DBMakerCLIConfig(BaseModel):
    """DBMaker CLI configuration."""
    dmsqlc_path: str
    database: str
...
class ServerConfig(BaseModel):
    ...
    dbmaker_cli: Optional[DBMakerCLIConfig] = None
...
def load_from_ini(file_path: str) -> ServerConfig:
    ...
    if 'DBMAKER_CLI' in config:
        cli_config = config['DBMAKER_CLI']

        dbmaker_cli = DBMakerCLIConfig(
        dmsqlc_path=cli_config.get('dmsqlc_path'),
        database=cli_config.get('database')
    )
    
    # Extract connection configs
    for section in config.sections():
        if section in ['SERVER', 'DBMAKER_CLI']:
            continue
...
def load_from_claude_config(claude_config_path: Optional[str] = None) -> Optional[ServerConfig]:
    ...
    try:
        ...
        if 'dbmaker_cli' in odbc_config:
            db_cli = odbc_config['dbmaker_cli']

            dbmaker_cli = DBMakerCLIConfig(
                dmsqlc_path=db_cli.get('dmsqlc_path'),
                database=db_cli.get('database')
            )
            
        #print("DBMaker CLI:", dbmaker_cli)
